...
public class CropView extends View {
    ...
    private void init(Context context) {
        ...
        mGuidelines = 1;
        setInitialAttributeValues(1, false, 1, 1);
    }
    ...
    public static boolean showGuidelines() {
        return abs(Edge.LEFT.getCoordinate() - Edge.RIGHT.getCoordinate()) >= 100.0F && abs(Edge.TOP.getCoordinate() - Edge.BOTTOM.getCoordinate()) >= 100.0F;
    }
    ...
    public CroppedRect getCropRect(int videoWidth, int videoHeight) {
        float left = Edge.LEFT.getCoordinate();
        float top = Edge.TOP.getCoordinate();
        float right = Edge.RIGHT.getCoordinate();
        float bottom = Edge.BOTTOM.getCoordinate();
        ...
        result.left = (int) (left * videoWidth / getWidth());
        result.right = (int) (right * videoWidth / getWidth());
        result.top = (int) (top * videoHeight / getHeight());
        result.bottom = (int) (bottom * videoHeight / getHeight());

        float width = abs((float) result.left - (float) result.right) / (float) videoWidth;
        float height = abs((float) result.top - (float) result.bottom) / (float) videoHeight;
        float x = (float) result.left / (float) videoWidth;
        float y = ((float) videoHeight - (float) result.bottom) / (float) videoHeight;

        return new CroppedRect(width, height, x, y);
    }
